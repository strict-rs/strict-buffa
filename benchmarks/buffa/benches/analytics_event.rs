#![allow(dead_code)]
include!("common.rs");
use bench_buffa::bench::{AnalyticsEvent, __buffa::view::AnalyticsEventView};
...
fn run(c: &mut Criterion) {
    ...
    g.bench_function("decode_view", |b| {
        b.iter(|| for p in &ds.payload { criterion::black_box(AnalyticsEventView::decode_view(p).unwrap()); })
    });
    g.finish();
}
